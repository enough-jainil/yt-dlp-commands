        ...
        return (
          <button
            ...
            className={cn(
              ...
              mode === key ? "bg-primary text-primary-foreground shadow-sm" : "hover:bg-muted hover:text-foreground",
            )}
          >
            <Icon className="h-4 w-4 mr-2" />
            <span>{config.label}</span>
            <span className="sr-only md:not-sr-only md:ml-1.5 text-xs opacity-70">({config.description})</span>
          </button>
        )
      })}
    </div>
